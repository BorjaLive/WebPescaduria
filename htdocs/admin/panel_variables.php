<html>
	...
	<body>
		<?php
			...
			if(!empty($_GET["mod"])){
				...
				if(empty($_POST["cesta"])){
					$link->query("DELETE FROM `vars` WHERE `clave`='cesta';");
						$link->query("INSERT INTO `vars` (`clave`,`valor`) VALUES ('cesta','2');");
				}
				$link->close();
		...
		<form action="panel_variables.php?mod=true" method="post">
			...
			<fieldset>
				<legend>Estado de la web</legend>
				<input type="checkbox" name="cesta" <?php echo $valores["cesta"]==1?'checked="checked"':"";?> value="1">Cesta abirta, se admiten pedidos.<br>
			</fieldset>
